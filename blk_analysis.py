...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Process PDF to outline blocks and extract text details"
    )
    parser.add_argument("input_file", help="Input PDF file")
    parser.add_argument("-o", "--output_file", help="Output PDF file")
    parser.add_argument(
        "-ob", "--outline_blocks", action="store_true", help="Outline blocks in the PDF"
    )
    parser.add_argument(
        "-oit",
        "--outline_images_tables",
        action="store_true",
        help="Outline images and tables in the PDF",
    )
    parser.add_argument(
        "-ad", "--appdir", help="Application directory", default="pdf_blocks"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.07,
        help="Header size as a percentage of the page height (e.g., 0.1 for 10%%)",
    )
    parser.add_argument(
        "-fs",
        "--footer_size",
        type=float,
        default=0.07,
        help="Footer size as a percentage of the page height (e.g., 0.1 for 10%%)",
    )
    parser.add_argument(
        "-main",
        "--main_pages",
        help='List of page ranges for the main document (e.g., "1-3,5,7-")',
    )
    parser.add_argument(
        "-exclude",
        "--exclude_pages",
        help='List of page ranges to exclude (e.g., "4,6,8-10")',
    )
    parser.add_argument(
        "-toc",
        "--toc_pages",
        help='List of page ranges for the table of contents (e.g., "2-3")',
    )
    parser.add_argument(
        "-cfg", "--config_file", help="Path to the YAML configuration file"
    )
    parser.add_argument(
        "-skip",
        "--skip_preprocessing",
        action="store_true",
        help="Skip the preprocessing phase and use the filtered data input file",
    )
    parser.add_argument(
        "--filtered_data_file", help="Path to the filtered data input file"
    )
    parser.add_argument(
        "-tcfg",
        "--toc_parsing_config",
        help="TOC parsing configuration to use from the global config file",
    )
    parser.add_argument(
        "-nf", "--nofiles", action="store_true", help="Do not save files at completion"
    )
...
def main():
    ...
    toc_parsing_config = global_config.get("toc_parsing_configurations", {}).get(
        args.toc_parsing_config or "default", {}
    )
    toc_parsing_config = common_regex | toc_parsing_config

    output_dir = (
        args.output_dir if args.output_dir else os.path.basename(args.input_file)[:-4]
    )
    app_dir = args.appdir
    ...
    output_file = (
        ...
        if args.output_file
            ...
            output_dir_path, f"{os.path.basename(args.input_file)[:-4]}_outline.pdf"
        )
    )

    if args.skip_preprocessing:
        filtered_data_file = (
            args.filtered_data_file
            if args.filtered_data_file
            else os.path.join(
                output_dir_path,
                f"filtered_{os.path.basename(args.input_file)[:-4]}_blocks.json",
            )
        )
        toc_data_file = os.path.join(
            output_dir_path, f"toc_{os.path.basename(args.input_file)[:-4]}_blocks.json"
        )
        ...
        if not args.nofiles:
            json_output_file = os.path.join(
                output_dir_path, f"{os.path.basename(args.input_file)[:-4]}_blocks.json"
            )
            with open(json_output_file, "w", encoding="utf-8") as f:
                json.dump(result["pages_data"], f, ensure_ascii=False, indent=4)

            filtered_json_output_file = os.path.join(
                output_dir_path,
                f"filtered_{os.path.basename(args.input_file)[:-4]}_blocks.json",
            )
            ...
            excluded_json_output_file = os.path.join(
                output_dir_path,
                f"excluded_{os.path.basename(args.input_file)[:-4]}_blocks.json",
            )
            with open(excluded_json_output_file, "w", encoding="utf-8") as f:
                json.dump(
                    result["excluded_pages_data"], f, ensure_ascii=False, indent=4
                )

            toc_json_output_file = os.path.join(
                output_dir_path,
                f"toc_{os.path.basename(args.input_file)[:-4]}_blocks.json",
            )
            with open(toc_json_output_file, "w", encoding="utf-8") as f:
                json.dump(toc_data, f, ensure_ascii=False, indent=4)

            with open(
                os.path.join(output_dir_path, "images.json"), "w", encoding="utf-8"
            ) as f:
                json.dump(result["images"], f, ensure_ascii=False, indent=4)

            with open(
                os.path.join(output_dir_path, "tables.json"), "w", encoding="utf-8"
            ) as f:
                json.dump(result["tables"], f, ensure_ascii=False, indent=4)

            with open(
                os.path.join(output_dir_path, "locations.json"), "w", encoding="utf-8"
            ) as f:
    ...
    sections = analyze_pdf(
        filtered_pages_data, analysis_config, section_text_dir, location_info
    )

    if not args.nofiles:
        sections_output_file = os.path.join(
            output_dir_path, f"{os.path.basename(args.input_file)[:-4]}_sections.json"
        )
