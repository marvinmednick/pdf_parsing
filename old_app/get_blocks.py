...
def parse_arguments():
    ...
    parser.add_argument(
        "--outline_blocks", action="store_true", help="Outline blocks in the PDF"
    )
    parser.add_argument(
        "-ad", "--appdir", help="Application directory", default="block_extract"
    )
...
def process_pdf(
    ...
    for page_num, page in enumerate(doc):
        page_images, page_tables, page_locations, doc_image_index, doc_table_index = (
    ...
    json_output_file = os.path.join(
        output_dir, f"{os.path.basename(input_file)[:-4]}_blocks.json"
    )
    with open(json_output_file, "w", encoding="utf-8") as f:
        json.dump(pages_data, f, ensure_ascii=False, indent=4)

    filtered_json_output_file = os.path.join(
        output_dir, f"filtered_{os.path.basename(input_file)[:-4]}_blocks.json"
    )
...
def main():
    ...
    output_file = (
        ...
        if args.output_file
            ...
            output_dir_path, f"{os.path.basename(args.input_file)[:-4]}_blocks.pdf"
        )
    )
